...
const runAction = () => {
    ...
    const skipSign = getInput("skip_sign") === "true";
    ...
    // Copy "github_release_token" input variable to "GITHUB_RELEASE_TOKEN" env variable (required by `electron-builder`)
    setEnv("GITHUB_RELEASE_TOKEN", getInput("github_release_token"));
    ...
    for (let i = 0; i < maxAttempts; i += 1) {
        try {
            run(
                `${useNpm ? "npx --no-install" : "yarn run"} ${cmd} --${platform} ${release ? "--publish always" : ""
                } ${args} ${skipSign ? "-c.mac.identity=null" : ""}`,
                appRoot,
            );
            break;
        } catch (err) {
